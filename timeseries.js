...
function generateDataArray() {
  ...
  for (let i = 0; i < RECORD_COUNT; i++) {
    ...
    data[arrayIndex].push({
      ...
      metadata: {
      type: faker.helpers.arrayElement(['type1']),
      }
    });
  }
  return data;
...
async function createMongoTimeSeriesCollection() {
  ...
  try {
    ...
    await db.collection(MONGO_COLLECTION).drop().catch(() => {});
    ...
    console.error('Error creating timeseries collection:', err);
  } finally {
    await client.close();
  }
}
...
async function main() {
  ...
  await createMongoTimeSeriesCollection()
